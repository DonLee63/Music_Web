document.addEventListener('DOMContentLoaded', () => {
  ...
  premiumcheck();
  let thempt = 0;
  document.getElementById('thembai').addEventListener('click', () => {
    if (thempt == 0) {
      hienphantu();
      thempt = 1;
      document.getElementById('thembaimoi').innerHTML = 'Complete  list';
    }
    else {
      anphantu1();
      document.getElementById('thembaimoi').innerHTML = 'Add new song';
      thempt = 0;
      sapxep();
      saveData1();
    }
  ...
  function sapxep() {
    for (let i = 1; i <= 75; i++) {
      if (listcuatoi[i - 1] == 1) {
        ...
        let pop = document.getElementById('pop-2');
        pop.appendChild(s);
      }
    }
  }
  // Sử dụng vòng lặp để tạo mã cho các phần tử s1 đến s75
  for (let i = 1; i <= 75; i++) {
    let s = document.getElementById(`s${i}`);
    let count = 0;
    s.addEventListener('click', () => {
      if (thempt == 1) {
        count++;
        let pop;
        if (count % 2 == 1 && listcuatoi[i - 1] == 1) {
          pop = document.getElementById('pop-3');
          listcuatoi[i - 1] = 0;
          count--;
        }
        else {
          if (count % 2 == 0 && listcuatoi[i - 1] == 1) {
            pop = document.getElementById('pop-2');
            listcuatoi[i - 1] = 1;
            count--;
          }
          else {
  ...
  function saveData1() {
    SharedData.setItem("myData1", JSON.stringify({ listcuatoi }));
  }
  function favourite() {
    if (yeuthich[0] == 0) {
      for (let i = 0; i < 100; i++) {
  ...
  function listcheck() {
    const data = SharedData.getItem("myData1");
    if (data) {
      let listcuatoi1 = Array();
      listcuatoi1 = JSON.parse(data).listcuatoi;
      for (let i = 0; i < 75; i++) {
        listcuatoi[i] = listcuatoi1[i];
      }
    }
  }
  // phần list favorite
  ...
  function anphantu1() {
    ...
    var elem1 = document.getElementById('yeucau');
  ...
  function premiumcheck() {
    if (premium == 1) { }
      ...
      document.getElementById('thembai').remove();
